...
IF DEFINED APPVEYOR (
    ...
    C:\ProgramData\chocolatey\lib\opencover.portable\tools\OpenCover.Console.exe -register:user -target:"c:\Program Files\dotnet\dotnet.exe" -targetargs:"test src\Playground.Calculator.Test\Playground.Calculator.Test.csproj -c Debug --no-build --logger \"trx;%APPVEYOR_BUILD_FOLDER%\LogFileName=TestResults.trx\"" -threshold:1 -oldStyle -returntargetcode -filter:"+[Playground*]* -[Playground*.Test]*" -excludebyattribute:*.ExcludeFromCodeCoverage* -excludebyfile:*\*Designer.cs -hideskipped:All -mergebyhash -mergeoutput -output:.\coverage-dotnet.xml
