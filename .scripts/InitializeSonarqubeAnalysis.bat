IF DEFINED APPVEYOR (
	...
	IF DEFINED APPVEYOR_PULL_REQUEST_NUMBER (
		...
		SoarQube.Scanner.MSBuild.exe begin    ^
		  /k:"PlaygroundDotNetCore" ^
		  /n:"PlaygroundDotNetCore" ^
		  /d:sonar.cs.opencover.reportsPaths="coverage-dotnet.xml" ^
		  /d:"sonar.cs.vstest.reportsPaths=%APPVEYOR_BUILD_FOLDER%\TestResults.trx" ^
		  /d:"sonar.organization=coenm-github" ^
		  /d:"sonar.host.url=https://sonarcloud.io" ^
		  /d:"sonar.login=%sonar_login%" ^
		  /v:%appveyor_build_version% ^
		  /d:"sonar.branch=%GitVersion_BranchName%" ^
		  /d:"sonar.cpd.cross_project=true"
